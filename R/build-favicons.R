...
#' `jamstack/favicon` and copied by [init_site()] to the relevant location when
#' the website is rebuilt.
#'
#' Once complete, you should add `jamstack/` to `.Rbuildignore ` to avoid a NOTE
#' during package checking.
#'
#' @inheritParams as_jamstack
...
build_favicons <- function(pkg = ".", overwrite = FALSE) {
  ...
  if (has_favicons(pkg) && !overwrite) {
    message("Favicons already exist in `jamstack/`. Set `overwrite = TRUE` to re-create.")
    return(invisible())
  }
  ...
  if (!identical(result$result$status, "success")) {
    stop(
      "API request failed. ", "
      Please submit bug report to <https://github.com/saiemgilani/jamstackingup/issues>",
      call. = FALSE
    )
  }
  ...
  tryCatch({
    utils::unzip(tmp, exdir = path(pkg$src_path, "jamstack", "favicon"))
  },
  warning = function(e) {
...
copy_favicons <- function(pkg = ".") {
  ...
  favicons <- path(pkg$src_path, "jamstack", "favicon")
...
has_favicons <- function(pkg = ".") {
  ...
  file.exists(path(pkg$src_path, "jamstack", "favicon"))
}
